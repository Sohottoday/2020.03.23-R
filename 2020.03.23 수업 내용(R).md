...
- R관련 정보를 구글에서 검색 시 xxxx in R 과 같은 형식으로 뒤에 in R을 붙여준다.
- 팩터
  - levels를 통해 범위 지정 가능
...
- 데이터프레임
  ...
  - 기본 저장은 팩터 형식으로 저장됨. 팩터형식으로 저장하지 않으려면 stringsAsFactors 값을 F로 준다.(stringsAsFactors = F)

  - 대괄호 안에 값을 하나만 주면 열의 인덱싱으로 취급한다.
  - 사용 빈도로는 df$컬럼이름, [열의 인덱싱], 그 외에는 비슷
